       ...
       | PStr structure ->
         (match structure |> Metadata.make with
          ...
          | Error
            ...
            Location.raise_errorf
              ~loc
              "`output` type for %s collection that is defined in `input` is not found \
               or in invalid state"
          ...
          | Error
            ...
            (match reason with
             | `DifferentIO (_input_type, _output_type) ->
               Location.raise_errorf
                 ~loc
                 "Validator for `%s` field is required because its input and output \
                  types are different. So validator function is required to produce \
                  value of output type from an input value."
                 (match field with
          ...
          | Error (IOMismatch (OutputFieldsNotInInput { fields })) ->
            (match fields with
             | [] ->
               failwith
                 "Empty list of non-matched fields in \
                  IOMatchError(OutputFieldsNotInInput)"
             | field :: [] | field :: _ ->
               Location.raise_errorf
                 ~loc
