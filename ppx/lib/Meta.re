...
module InputTypeParser = {
  ...
  let parse = (~decl, ~structure, ~loc as _, fields) => {
    let entries =
      fields
      |> List.rev
      |> List.fold_left(
    ...
    |> List.fold_left(
         (res, entry: validated_entry) =>
           ...
           | (
             ...
             |> List.fold_left(
                  (
                    _res: option(InputField.validated),
  ...
  let find_field =
    ...
    |> List.fold_left(
         (res, validator) =>
           ...
           | (
               None,
               ValidatedInputFieldOfCollection({collection: _, field: _}),
               ({txt: _}, _),
             ) =>
             None
    ...
    |> List.iter(
         ...
         | {pstr_desc: Pstr_type(_rec_flag, decls)} => {
             ...
             |> List.iter(
                  ...
                  // Message type
                  | {ptype_name: {txt: "message"}, ptype_loc: _} =>
                    message_type := Some()

                  // Metadata type
                  | {ptype_name: {txt: "metadata"}, ptype_loc: _} =>
                    metadata := Some()
