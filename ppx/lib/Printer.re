...
module FieldPrinter = {
  let update_action = (~field) =>
    "Update" ++ (field |> String.capitalize_ascii) ++ "Field";

  let blur_action = (~field) =>
    "Blur" ++ (field |> String.capitalize_ascii) ++ "Field";
...
module CollectionPrinter = {
  let fields_statuses_type = (collection: Collection.t) =>
    collection.singular ++ "FieldsStatuses";

  let validator_type = (collection: Collection.t) =>
    collection.plural ++ "Validators";
  ...
  let result_value = (collection: Collection.t) =>
    collection.plural ++ "Result";
};
